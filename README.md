...
- **Repo hygiene** — issue/PR templates, `SECURITY.md`, `SUPPORT.md`, `CODEOWNERS`, `FUNDING`, `LICENSE`, `.gitattributes`, `.dockerignore`, a badge-rich README, VS Code launch configs (current file, tests, attach, entry points), and a `.zed/settings.json` (ruff formatter/LSP, format-on-save); always-on Commitizen and git hooks (run via prek) and an always-on `CITATION.cff` with a validation workflow.
