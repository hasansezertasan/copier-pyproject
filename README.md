...
- **CI/CD & release** — matrix tests on macOS/Linux/Windows (full Python 3.10-3.14
  depth on Linux, one representative interpreter elsewhere, and no CI spend at all
  while a PR is a draft), trusted-publishing to PyPI, and release automation via release-please, with PR title linting, linked-issue enforcement, and a PR task-list completion check.
