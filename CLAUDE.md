...
8. **Workflow hardening + zizmor** (always included). Every generated workflow
   ...
   `uses:`):
   - **`persist-credentials: false`** on every `actions/checkout` — *except* jobs
     that `git push` with the checkout credential (`gh-pages.yml` and
     release-please's `deploy-docs`, both running `ghp-import -p`), which instead
     carry a justified `# zizmor: ignore[artipacked]`.
